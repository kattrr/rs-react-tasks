...
const PokemonDetailsPanel = ({ detailsName, onClose }: PokemonDetailsPanelProps) => {
  ...
  return (
    <div className="w-1/3 bg-white rounded-2xl shadow-lg p-6 flex flex-col relative min-h-[725px]" data-testid="details-panel">
      <button
        onClick={onClose}
      ...
      {details && !loading && !error && (
        <div className="flex flex-col items-center text-indigo-950">
          ...
          <p className="mb-2">Abilities: {details.abilities?.map((a) => a.ability.name).join(', ')}</p>
          <p className="mb-2">Forms: {details.forms?.map((f) => f.name).join(', ')}</p>
