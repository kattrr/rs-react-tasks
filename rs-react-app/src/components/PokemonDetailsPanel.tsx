...
const PokemonDetailsPanel = ({ detailsName, onClose }: PokemonDetailsPanelProps) => {
  ...
  return (
    <div className="w-1/3 bg-white rounded-2xl shadow-lg p-6 flex flex-col relative min-h-[725px]">
      <button
        onClick={onClose}
      ...
      {details && !loading && !error && (
        <div className="flex flex-col items-center text-indigo-950">
          ...
          <h2 className="text-2xl font-bold mb-2 text-indigo-950">{details.name}</h2>
          <p className="mb-2">Type: {details.types.map((t) => t.type.name).join(', ')}</p>
          <p className="mb-2">Height: {details.height}</p>
          <p className="mb-2">Abilities: {details.abilities?.map((a: any) => a.ability.name).join(', ')}</p>
          <p className="mb-2">Forms: {details.forms?.map((f: any) => f.name).join(', ')}</p>
          <div className="mb-2 max-h-32 w-full">
            <span className="font-semibold">Moves:</span>
            <ul className="list-disc list-inside text-sm ">
              {details.moves?.slice(0, 10).map((m: any) => (
